...
const content = fs.readFileSync('../week1/data/m01.txt');
...
$('tr').each(function(i, elem) {
  ...
  if ($(elem).attr('style') == 'margin-bottom:10px') {
    ...
    let groupName = $('b', firstTD).text();
      ...
      // DATES
      let meetingList = secondTD.html().trim()
    //   console.log(meetingList)
      meetingList = secondTD.html().trim().split('<br>\n                    \t<br>')
      // console.log(meetingList)
      ...
      meetingList = meetingList.forEach((e, j) => {
        ...
        if (e.length > 0){
          ...
          e = e.replace("<br>","\n")
          if (e.includes('<b>Meeting Type</b>')){
            if (e.includes('<br>')){
              console.log(i, j, e.split('<b>Meeting Type</b>')[1].split('<br>')[0]);
            } else {
              console.log(i, j, e.split('<b>Meeting Type</b>')[1].split('\n')[0]);
            }
          }
          
          if (e.includes('<b>Special Interest</b>')){
            console.log(i, j, e.split('<b>Special Interest</b>')[1].split('\n')[0]);
          }
        }
      }) */
      let days = secondTD;
